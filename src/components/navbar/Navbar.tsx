...
const Navbar: React.FC = () => {
  ...
  return (
    <nav className="shadow-md">
      <div className="container mx-auto flex justify-between items-center py-4 text-cyan-950">
        {/* Logo & Links */}
...
{/*           
          <div className="border-dashed border-2 border-red-600 rounded-full h-7 flex items-center">
            DarkMode 
          </div> */}

          <button className="px-3 py-1" onClick={() => setIsSignupOpen(true)}>Signup</button>
          <button
          ...
          <button className="md:hidden p-2" onClick={() => setIsSidebarOpen(!isSidebarOpen)}>☰</button>
        </div>
      </div>

      {/* Sidebar */}
      {isSidebarOpen && (
        <aside className="fixed top-0 right-0 w-64 h-full bg-white shadow-lg p-4 z-50 transition-all">
          <button
          ...
          <div className="mt-10 text-center">
            <Link href="/student/profile" onClick={() => setIsSidebarOpen(false)}>
              <Image
          ...
          <ul className="flex flex-col mt-8 gap-10">
            ...
            <li>
              <button onClick={() => { handleScrollToCourses(); setIsSidebarOpen(false); }}>
                Courses
              </button>
            </li>
